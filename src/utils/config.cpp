...
//一个Section会有一个Content链表，会有自己名字，也会形成一个链表，链表套链表
struct Section {
    char *name;
    Content *contents;
    Section *next;
...
//很nice，递归删除记录，释放内存，最初传入是section的头指针
...
//函数套在循环里，循环拆每一行，构成section和content链表
...
void Config::read_config(const char *filename) {
    ...
    while (err == NO_ERROR) {
        fp.readline(cache, CACHE_SIZE);//读一行，读到字符数组中
        if (fp.is_eof()) break;
...
//找到key对应的value
const char* Config::get_string(const char *sec, const char *key, const char *deft) {
    const char *value = deft;
    //查看配置文件是否发生变动，发生变动就重新加载配置文件，_monitor是config的内部对象
    _monitor.check();
...
int Config::get_int(const char *sec, const char *key, const int deft) {
    const char *value = get_string(sec, key, NULL);
    if (value == NULL) return deft;
    return atoi(value);//字符串转数字
}
