...
void send_test(char *host, int port)
{
    ...
    if (conn->has_data() > 0) {
        RawData *rdata = conn->recv_data();//获取接收的数据
        printf("Client received: %s\n", rdata->data());
        assert(strcmp(rdata->data(), iamserver) == 0);//进行比较
        delete rdata;
    }
...
static void start_server(int port) {
    ...
    RawData data(32);//申请数据空间
    data.pack(iamserver, strlen(iamserver) + 1);//打包数据
    conn->send_data(&data);//通过连接发送数据
